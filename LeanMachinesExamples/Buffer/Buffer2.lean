...
import LeanMachinesExamples.Buffer.Buffer1

/-!

## Third refinement step

In this third step, the fetching of elements is made according
to a priority scheme. Since this does not follow the structural
induction principle of lists, some proof details are a little bit
"hairy" (and, presumably, *not* automation-friendly).

Note that this is both an algorithm and data-refinement step.

The machine is fully functional/deterministic.
...
/-- The state of the concrete Buffer machine. -/
...
Since elements are present both a the concrete and the abstract level,
 we may (try to) use the stronger form of functional refinement.
While *lifting* to the abstract level is easy,  the *unlifting*
part of strong refinement is more complex, requring a few
functions and associated properties.
-/


/-- Lifting the concrete state to the abstract, forgetting about priorities. -/
@[simp]
...
/-- Injecting the abstract changes to the buffer using low priorities as
 a default. This way abstract events can be potentially reused at the
 concrete level. -/
@[simp]
...
/-- Instantiation of strong functional refinement. -/
...
/-- Event: A refinement of `B1.Put` that simply reuses the latter.
The only proof obligation is that the data refinement does not
contradict with the abstrat event, taking advantage of strong refinement.
 -/
...
/-- An alternative refinement of `B1.Put`, but this time with an explicit
specification of a priority.
-/
...
Retrieving elements according to priorities is less trivial.
Below we define a bunch of functions and associated theorems that
will be used to define the event.
-/
...
/-- Event: Retrieving an element according to priorities.
This is a (non-trivial) alternative refinement of `B1.Fetch`.
-/
